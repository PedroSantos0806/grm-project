...
function authMiddleware(req, res, next) {
    const authHeader = authMiddleware = req.headers['authorization'];
...
const alunoSchema = Joi.object({
    ...
    altura: Joi.number().precision(2).allow(null,''),
...
app.post('/pagamentos', authMiddleware, async (req, res) => {
    ...
    if (req.user.tipo !== 'admin') {
        const [aluno] = await pool.query('SELECT * FROM alunos WHERE id = ? AND responsavel_id = ?', [alunoId, req.user.id]);
        if (!aluno.length) {
          return res.status(403).json({ message: 'Acesso negado' });
        }
      }
...
app.get('/alunos/:alunoId/pagamentos', authMiddleware, async (req, res) => {
    ...
    try {
        let whereClause = 'alunoId = ?';
        let values = [alunoId];

        if (req.user.tipo !== 'admin') {
            whereClause += ' AND responsavel_id = ?';
            values.push(req.user.id);
        }

        const [rows] = await pool.query(`SELECT * FROM pagamentos WHERE ${whereClause}`, values);
        res.json(rows);
    } catch (error) {
        console.error(error);
